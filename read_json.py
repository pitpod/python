...
#output_contacts = open('./output.vcf', 'w', encoding='SHIFT-JIS')
output_contacts = open('./output-3.vcf', 'w', encoding='utf-8')
...
for i in range(0, len(json_load)):
    ...
    # output_contacts.write('VERSION:2.1\n')
    firstname = json_load[i]['structuredName']['data1']
    lastname = json_load[i]['structuredName']['data3']
    output_contacts.write(f"N:{lastname};{firstname}\n")
    output_contacts.write(f"FN:{firstname} {lastname}\n")
    output_contacts.write(f"X-PHONETIC-LAST-NAME:{json_load[i]['structuredName']['data9']}\n")
    output_contacts.write(f"TEL;type=CELL;type=VOICE;type=pref:{json_load[i]['phone'][0]['data1']}\n")
