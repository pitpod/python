...
def sql_data():
    ...
    monthNo = int(input("月："))
    dayNo = int(input("日："))
    excel_dateNo = Excel_date(dt.datetime(yearNo, monthNo, dayNo))
    sqlStr = f"SELECT time,name,place1,place2 FROM dayly_data_{yearNo} where date={excel_dateNo}" + \
...
dateNo = f'{yearMontDayNo[0]}_{yearMontDayNo[1]}_{yearMontDayNo[2]}'
